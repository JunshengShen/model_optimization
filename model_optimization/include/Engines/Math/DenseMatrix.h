...
public:
    ...
    bool ElementsValid();
    T Determinant();
    void ExtractRow(UINT Row, suVector<T> &Result);
    void ExtractCol(UINT Col, suVector<T> &Result);
    suVector<T> ExtractRow(UINT Row);
    suVector<T> ExtractCol(UINT Col);
    ...
    void LUSolve(suVector<T> &x, const suVector<T> &b);
    bool EigenSystem(suVector<T> &Eigenvalues, DenseMatrix<T> &Eigenvectors) const;
    bool EigenSystem(T *Eigenvalues, T **Eigenvectors) const;
    String EigenTest(const suVector<T> &Eigenvalues, const DenseMatrix<T> &Eigenvectors) const;
    String EigenTest(const T *Eigenvalues, const T **Eigenvectors) const;

    bool VictorEigenSystem(suVector<T> &Eigenvalues, DenseMatrix<T> &Eigenvectors) const;
    
    //
    // Conversion
    //
    /*void ToColumnVector(suVector<T> &Result)
    {
    ...
    static void Multiply(suVector<T> &Result, const DenseMatrix<T> &Left, const suVector<T> &Right);
    static void MultiplyInPlace(DenseMatrix<T> &Result, T Right);
    static DenseMatrix<T> OuterProduct(const suVector<T> &A, const suVector<T> &B);
