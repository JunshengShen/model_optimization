...
void KDTree3::BuildTree(const suVector<Vec3f> &Points)
{
    FreeMemory();
...
void KDTree3::KNearest(const Vec3f &Pos, UINT k, suVector<UINT> &Result, float Epsilon)
{
...
void KDTree3::WithinDistance(const Vec3f &Pos, float Radius, suVector<UINT> &Result) const
{
    for(UINT ElementIndex = 0; ElementIndex < 3; ElementIndex++)
    {
        queryPt[ElementIndex] = Pos[ElementIndex];
