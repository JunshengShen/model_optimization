...
public:
    ...
    void BuildTree(const suVector<Vec3f> &Points);
    void KNearest(const Vec3f &Pos, UINT k, suVector<UINT> &Result, float Epsilon);
    void KNearest(const Vec3f &Pos, UINT k, UINT *Result, float Epsilon);
    void WithinDistance(const Vec3f &Pos, float Radius, suVector<UINT> &Result) const;
