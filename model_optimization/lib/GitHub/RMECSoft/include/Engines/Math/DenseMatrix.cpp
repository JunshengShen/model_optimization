...
template<class T>
void DenseMatrix<T>::ExtractRow(UINT Row, suVector<T> &Result)
{
...
template<class T>
void DenseMatrix<T>::ExtractCol(UINT Col, suVector<T> &Result)
{
...
template<class T>
suVector<T> DenseMatrix<T>::ExtractRow(UINT Row)
{
    suVector<T> Result(_ColCount);
    for(UINT Col = 0; Col < _ColCount; Col++)
    {
        Result[Col] = _Data[Row * _ColCount + Col];
    }
...
template<class T>
suVector<T> DenseMatrix<T>::ExtractCol(UINT Col)
{
    suVector<T> Result(_RowCount);
    for(UINT Row = 0; Row < _RowCount; Row++)
    {
        Result[Row] = _Data[Row * _ColCount + Col];
    }
...
template<class T>
DenseMatrix<T> DenseMatrix<T>::OuterProduct(const suVector<T> &A, const suVector<T> &B)
{
...
template<class T>
void DenseMatrix<T>::LUSolve(suVector<T> &x, const suVector<T> &b)
{
...
template<class T>
bool DenseMatrix<T>::VictorEigenSystem(suVector<T> &Eigenvalues, DenseMatrix<T> &Eigenvectors) const
{
...
template<class T>
bool DenseMatrix<T>::EigenSystem(suVector<T> &Eigenvalues, DenseMatrix<T> &Eigenvectors) const
{
    const UINT Dimension = _RowCount;
	Eigenvalues.Allocate(Dimension);
	Eigenvectors.Allocate(Dimension, Dimension);
	suVector<T*> EigenvectorList(Dimension);
...
template<class T>
String DenseMatrix<T>::EigenTest(const suVector<T> &Eigenvalues, const DenseMatrix<T> &Eigenvectors) const
{
    const UINT Dimension = _RowCount;
	suVector<const T*> EigenvectorList(Dimension);
	for(UINT DimensionIndex = 0; DimensionIndex < Dimension; DimensionIndex++)
	{
		EigenvectorList[DimensionIndex] = Eigenvectors[DimensionIndex];
	}
...
String DenseMatrix<T>::EigenTest(const T *Eigenvalues, const T **Eigenvectors) const
{
    ...
    String Description;
    PersistentAssert(Square() && _RowCount >= 2, "Invalid matrix dimensions");
    suVector<T> Eigenvector(_RowCount), Result;
    double MaxError = 0.0;
...
template<class T>
void DenseMatrix<T>::Multiply(suVector<T> &Result, const DenseMatrix<T>&Left, const suVector<T> &Right)
{
...
void DenseMatrix<T>::Multiply(DenseMatrix<T>&Result, const SparseMatrix<T> &Left, const DenseMatrix<T> &Right)
{
    ...
    for(UINT RowIndex = 0; RowIndex < RowCount; RowIndex++)
    {
        const suVector< SparseElement<T> > &LeftRowEntries = Left.Rows()[RowIndex].Data;
...
void DenseMatrix<T>::Multiply(DenseMatrix<T>&Result, const DenseMatrix<T>&Left, const SparseMatrix<T> &Right)
{
    ...
    for(UINT RowIndex = 0; RowIndex < RowCount; RowIndex++)
    {
        for(UINT LeftColIndex = 0; LeftColIndex < InnerCount; LeftColIndex++)
        {
            ...
            const suVector<SparseElement> &RightRowEntries = Right.Rows()[LeftColIndex].Data;
