...
void KDTree::BuildTree(const suVector<suVector<double> > &Points, UINT Dimension, UINT MaxK)
{
...
void KDTree::BuildTree(const suVector<suVector<float> > &Points, UINT Dimension, UINT MaxK)
{
    UINT PointCount = Points.Length();
    _Dimension = Dimension;
    _MaxK = MaxK;
...
#pragma omp parallel for
    ...
    {
        for(int ElementIndex = 0; ElementIndex < (int)_Dimension; ElementIndex++)
        {
            _DataPts[i][ElementIndex] = Points[i][ElementIndex];
        }
    }
...
void KDTree::KNearest(const float *QueryPoint, UINT k, suVector<UINT> &Indices, float Epsilon) const
{
    for(UINT ElementIndex = 0; ElementIndex < _Dimension; ElementIndex++)
    {
...
void KDTree::KNearest(const float *QueryPoint, UINT k, suVector<UINT> &Indices, suVector<float> &Distances, float Epsilon) const
{
    for(UINT ElementIndex = 0; ElementIndex < _Dimension; ElementIndex++)
    {
        _QueryPt[ElementIndex] = QueryPoint[ElementIndex];
...
void KDTree::KNearest(const double *QueryPoint, UINT k, suVector<UINT> &Indices, float Epsilon) const
{
    for(UINT ElementIndex = 0; ElementIndex < _Dimension; ElementIndex++)
    {
        _QueryPt[ElementIndex] = QueryPoint[ElementIndex];
...
void KDTree::KNearest(const double *QueryPoint, UINT k, suVector<UINT> &Indices, suVector<double> &Distances, float Epsilon) const
{
    for(UINT ElementIndex = 0; ElementIndex < _Dimension; ElementIndex++)
    {
        _QueryPt[ElementIndex] = QueryPoint[ElementIndex];
