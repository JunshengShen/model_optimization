                ...
                /*const*/ Vec3d& s)
{
    ...
    REAL pqrs=orient3d(p.v, q.v, r.v, s.v);
    ...
    REAL pqr=orient2d(p.v, q.v, r.v);
    if(pqr<0) return false;
    REAL prs=orient2d(p.v, r.v, s.v);
    if(prs<0) return false;
    REAL psq=orient2d(p.v, s.v, q.v);
                      ...
                      const BoundingBox& box)
{
    ...
    REAL s=0, t=1; // bounds on parameter for intersection interval
    for(unsigned int i=0; i<3; ++i){
        if(p[i]<q[i]){
            REAL d=q[i]-p[i];
            REAL s0=lo*(box.xmin[i]-p[i])/d, t0=hi*(box.xmax[i]-p[i])/d;
            if(s0>s) s=s0;
            if(t0<t) t=t0;
        }else if(p[i]>q[i]){
            REAL d=q[i]-p[i];
            REAL s0=lo*(box.xmax[i]-p[i])/d, t0=hi*(box.xmin[i]-p[i])/d;
            if(s0>s) s=s0;
            if(t0<t) t=t0;
        }else{
...
struct MeshObject
{
    ...
    bool
    inside(/*const*/ REAL point[3]) const
    {
        Vec3d p(point);
...
bool
point_inside_mesh(/*const*/ REAL point[3],
...
bool
segment_intersects_mesh(const REAL point0[3],
                        const REAL point1[3],
                        const MeshObject *mesh,
                        int *triangle_index,
                        REAL *s,
