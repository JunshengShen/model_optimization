...
//   glEnable( GL_CULL_FACE );
  ...
  set_scene(OpenMesh::Vec3f(0.0, 0.0, 0.0), 1.0);
...
void
GlutExaminer::set_scene( const OpenMesh::Vec3f& _cog, float _radius )
{
  center_ = _cog;
  radius_ = _radius;
...
bool
GlutExaminer::map_to_sphere(const OpenMesh::Vec2i& _v2D, OpenMesh::Vec3f& _v3D)
{
...
GlutExaminer::rotation(int x, int y)
{
  if (last_point_ok_) 
  {
	...
	new_point_2D = OpenMesh::Vec2i(x, y);
...
GlutExaminer::zoom(int x, int y)
{
  float dy = y - last_point_2D_[1];
  float h  = height_;
  translate(OpenMesh::Vec3f(0.0, 0.0, radius_ * dy * 3.0 / h));
}
...
void
GlutExaminer::translate(const OpenMesh::Vec3f& _trans)
{
  glLoadIdentity();
...
void
GlutExaminer::rotate(const OpenMesh::Vec3f& _axis, float _angle)
{
